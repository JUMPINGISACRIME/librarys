...
local themes = {
	preset = {
		["accent"] = rgb(34, 90, 215),
		["high_contrast"] = rgb(22, 22, 22),
		["low_contrast"] = rgb(15, 15, 15),
		["inline"] = rgb(35, 35, 35),
		["outline"] = rgb(0, 0, 0),
...
if old_library then
	for _, gui in next, old_library.guis or {} do
		pcall(function()
			gui:Destroy()
		end)
	end
	for _, connection in next, old_library.connections or {} do
		pcall(function()
			connection:Disconnect()
		end)
	end
	for _, drawing in next, old_library.drawings or {} do
		pcall(function()
			drawing:Remove()
		end)
	end
...
function library:panel(options)
	local cfg = {
		...
		size = options.size or dim2(0, 590, 0, 590),
...
local tint_sgui = library:create("ScreenGui", {
	Enabled = true,
	Parent = gethui(),
	Name = "",
	DisplayOrder = 1,
...
local tint_overlay = library:create("Frame", {
	Parent = tint_sgui,
	Active = false,
	Selectable = false,
...
library.tint_overlay = tint_overlay
...
run.RenderStepped:Connect(function()
	library.rainbow_hue = math.abs(math.sin(tick() * 0.3))
	for _, picker in pairs(library.rainbow_colorpickers) do
		if flags[picker.flag .. "_RAINBOW_FLAG"] then
			picker.set(hsv(library.rainbow_hue, picker.s, picker.v), picker.a)
		end
	end
end)
...
function library:window(properties)
	...
	function window.set_menu_visibility(bool)
		...
		if library.tint_overlay then
			library.tint_overlay.Visible = bool
		end
		if library.current_element_open then
			library.current_element_open.set_visible(false)
			library.current_element_open.open = false
	...
	local keybind_bg_gradient = library:create("UIGradient", {
		Parent = background,
		Name = "",
		Enabled = false,
		Rotation = 90,
	...
	library:apply_theme(keybind_bg_gradient, "contrast", "Color")
	...
	local keybind_list_gradient = library:create("UIGradient", {
		Parent = background,
		Name = "",
		Enabled = true,
		Rotation = 90,
	...
	library:apply_theme(keybind_list_gradient, "contrast", "Color")
	library:create("UIListLayout", {
		Parent = background,
		Name = "",
		Padding = dim(0, -1),
	...
	local main_window = library:panel({
		...
		size = (properties and properties.size) or dim2(0, 530, 0, 600),
		position = dim2(0, (camera.ViewportSize.X / 2) - 250, 0, (camera.ViewportSize.Y / 2) - 370),
		image = "rbxassetid://98823308062942",
		visible = false
	})
	local items = main_window.items
	window["title_text"] = items.text
	...
	local watermark = library:watermark({
		default = library.directory .. " | " .. os.date('%I:%M %p'),
	})
	local fpsValues = {}
	...
	task.spawn(function()
		...
		while task.wait() do
			pcall(function()
				...
				table.insert(fpsValues, delta > 0 and (1 / delta) or 0)
				...
				if currentTime - lastFpsUpdate >= fpsUpdateInterval then
					...
					watermark.change_text(string.format(library.directory .. " | " .. library.discord .. " | %d FPS | %d Players | %s", avgFps, playerCount, os.date('%I:%M %p')))
	...
	function window:UpdateTitle(text)
...
function library:watermark(options)
	local cfg = {
		default = options.text or options.default or os.date(library.directory .. " | %b %d %Y | %H:%M")
	}
...
function library:toggle(options)
	...
	local last_toggle_click = 0
	local function do_toggle_click()
		local now = tick()
		if now - last_toggle_click < 0.05 then
			return
		end
	...
	library:connection(toggle_holder.MouseButton1Click, do_toggle_click)
	cfg.set(cfg.default)
	cfg.set_element_visible(cfg.visible)
	library.config_flags[cfg.flag] = cfg.set
	library.visible_flags[cfg.flag] = cfg.set_element_visible
...
function library:colorpicker(options)
	...
	local info_section = library:create("Frame", {
		Parent = main_holder_background,
		Name = "info_section",
		AnchorPoint = vec2(0, 1),
	...
	library:apply_theme(info_section, "inline", "BackgroundColor3")
	local info_outline = library:create("Frame", {
		Parent = info_section,
		Name = "outline",
		Position = dim2(0, 1, 0, 1),
		Size = dim2(1, -2, 1, -2),
	...
	library:apply_theme(info_outline, "outline", "BackgroundColor3")
	local info_inner = library:create("Frame", {
		Parent = info_outline,
		Name = "inner",
		Position = dim2(0, 1, 0, 1),
		Size = dim2(1, -2, 1, -2),
	...
	local rgb_label = library:create("TextLabel", {
		Parent = info_inner,
		Name = "rgb_label",
		Position = dim2(0, 19, 0, -137),
		Size = dim2(1, -12, 0, 14),
		BackgroundTransparency = 1,
	...
	library:apply_theme(rgb_label, "text", "TextColor3")
	...
	function cfg.set_visible(bool)
		...
		if bool then
			...
			colorpicker_holder.Position = dim2(0, mouse_pos.X - (colorpicker_holder.AbsoluteSize.X / 2), 0, mouse_pos.Y + 10)
		end
	end
	...
	function cfg.set(color, alpha)
		...
		if library.rainbow_colorpickers[cfg.flag] then
			library.rainbow_colorpickers[cfg.flag].s = s
			library.rainbow_colorpickers[cfg.flag].v = v
			library.rainbow_colorpickers[cfg.flag].a = a
		end
		...
		if rgb_label then
			rgb_label.Text = string.format("(%d, %d, %d)", math.floor(cfg.color.R * 255), math.floor(cfg.color.G * 255), math.floor(cfg.color.B * 255))
		end
	...
	library.rainbow_colorpickers[cfg.flag] = {
		flag = cfg.flag,
		set = cfg.set,
		s = s,
		v = v,
...
function library:keybind(options)
	...
	function cfg.set(input)
		...
		if cfg.name and cfg.name ~= "Menu" and KEYBIND_ELEMENT then
			KEYBIND_ELEMENT.Visible = true
			if cfg.active then
				KEYBIND_ELEMENT.TextColor3 = themes.preset.accent
				library:apply_theme(KEYBIND_ELEMENT, "accent", "TextColor3")
			else
				KEYBIND_ELEMENT.TextColor3 = themes.preset.text
				library:apply_theme(KEYBIND_ELEMENT, "text", "TextColor3")
			end
			local text = tostring(cfg.key) ~= "Enums" and (keys[cfg.key] or tostring(cfg.key):gsub("Enum.", "")) or nil
			local __text = text and (tostring(text):gsub("KeyCode.", ""):gsub("UserInputType.", ""))
			if cfg.name then
				KEYBIND_ELEMENT.Text = "[" .. string.upper(string.sub(cfg.mode, 1, 1)) .. string.sub(cfg.mode, 2) .. "] " .. cfg.name .. " - " .. string.upper(__text or "none")
			end
		end
	end
...
function library:playerlist(options)
	...
	function cfg.create_player(player)
		...
		local priority_text = library:create("TextLabel", {
			...
			Name = "",
			FontFace = library.font,
			TextColor3 = tostring(player) ~= lp.Name and themes.preset.text or rgb(180, 180, 180),
			BorderColor3 = rgb(0, 0, 0),
		...
		TextButton.MouseButton1Click:Connect(function()
			...
			if cfg.labels.name then
				local player = players[player_name.Text]
				cfg.labels.name.set("User: " .. player_name.Text)
				cfg.labels.display.set("DisplayName: " .. player.DisplayName)
				cfg.labels.uid.set("User Id: " .. player.UserId)
			end
		end)
		return path
	end
